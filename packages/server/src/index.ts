...
import { listTracesSchema, listTraces, readTraceSchema, readTrace } from './tools/trace.js';
import { generateTestSpecSchema, generateTestSpec } from './tools/test-spec.js';
...
server.tool(
  'get_test_results',
  'Fetch detailed results of a completed batch test run. Returns pass/fail verdicts for subagent routing, action sequences, and response quality. (Subagent was called "topic" before Agent Script v2.0.)',
...
server.tool(
  'get_agent_metadata',
  'Retrieve complete agent configuration: subagents (with descriptions), actions per subagent, and agent structure. Use this to understand what the agent can do before generating tests. (Subagent was called "topic" before Agent Script v2.0, April 2026.)',
...
server.tool(
  'load_config',
  'Load user expectations from sfagent-config.yaml. Returns subagent-specific rules, global rules, and custom test scenarios. If no config file found, returns a sample template. Call this before generating tests to incorporate user expectations.',
...
// Tool: generate_test_spec
server.tool(
  'generate_test_spec',
  'Convert an exploratory session transcript into a YAML test spec compatible with `sf agent test run-eval`. Pairs Claude/Codex-driven exploration with Salesforce-native regression. Call before end_session so the transcript is still available.',
  generateTestSpecSchema.shape,
  async (args) => generateTestSpec(generateTestSpecSchema.parse(args))
);
