...
export async function listOrgs() {
  ...
  const formatted = orgs.map((org) => {
    const tags: string[] = [];
    if (org.isScratch) tags.push('scratch');
    if (org.isSandbox) tags.push('sandbox');
    if (org.isDevHub) tags.push('devhub');
    if (!org.isScratch && !org.isSandbox && !org.isDevHub) tags.push('production');
