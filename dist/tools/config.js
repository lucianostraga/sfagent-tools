...
function findConfigFile(startDir) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (parent === searchDir || parse(parent).root === parent)
            break;
        searchDir = parent;
    }
    return null;
...
function parseYamlSimple(content) {
    ...
    for (const rawLine of lines) {
        ...
        if (scalarMatch && !line.startsWith(' ') && !line.startsWith('\t')) {
            ...
            if (key === 'agent')
                config.agent = value.trim().replace(/^["']|["']$/g, '');
            if (key === 'targetOrg')
                config.targetOrg = value.trim().replace(/^["']|["']$/g, '');
            currentSection = null;
            currentTopic = null;
        ...
        if (sectionMatch && !line.startsWith(' ') && !line.startsWith('\t')) {
            ...
            inRules = false;
            inMessages = false;
            if (currentSection === 'expectations' && !config.expectations)
                config.expectations = [];
            if (currentSection === 'globalRules' && !config.globalRules)
                config.globalRules = [];
            if (currentSection === 'customScenarios' && !config.customScenarios)
        ...
        if (currentSection === 'customScenarios') {
            ...
            if (inMessages && line.match(/^\s+-\s+/)) {
                const value = line.replace(/^\s+-\s+/, '').replace(/^["']|["']$/g, '').trim();
                if (currentScenario)
                    currentScenario.messages.push(value);
                continue;
            }
            // Expect field
